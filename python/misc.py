...
def coord_grid(dat, nam, coor, step, plim, trlim, dlim, con, a=1, h=1, arp=False):
    ...
    for j in range(len(lats)): 
        ...
        for i in range(len(lons)):
            ...
            tr_lat.append(data['trend'].mean()*3652.5) 
            d_lat.append(data['d'].mean()) 
    ...
    levels = np.linspace(trlim[0],trlim[1],con)
    if arp == False: 
        #im = ax21.contourf(lons, lats, tr, levels, cmap=plt.cm.autumn.reversed(), transform=ccrs.PlateCarree())
        im = ax21.contourf(lons, lats, tr, levels, cmap=plt.cm.autumn.reversed(), transform=ccrs.PlateCarree(), extend="max")
        im.cmap.set_over(color="darkred")
    ...
    if arp == False: fig.savefig('ecmwf/' + nam + '/coord_2side_' + nam + '_' + co + '.png', format='png', dpi=240, bbox_inches="tight")
...
def plot_pc_var(vfr, pcl, s=20, sv=None):
    ...
    a_ = 0.7
    f, ax = plt.subplots(1, 1, figsize=(a_*6, a_*4), sharex=True)
    ...
    ax.legend(loc='upper right', fontsize=s-4, borderaxespad=0.54, framealpha=0.95)
    ax.grid(alpha=0.3)
...
def country_all_plots(f, mask, nam, per=['1979-01-01', '2022-12-31'], st='50', step=1, ant=False, 
                      ant2='east', ant2_av=False, proj='cartesian', seq='eig', pc_range=51):

    _, _, _, _ = eof_plot(f, nam, mask, proj=proj, a=1, b=1.5, per=per, st=st, step=step, ant=ant, ant2=ant2) 

    eofm_wa, var_wa, pc_wa = eof_pcs(f, nam, mask, pcs=list(range(1,pc_range)), npcs=51, 
                                weight=True, per=per, step=step, ant=ant, ant2=ant2) 
    dat_wa, _ = add_pc(pc_wa, var_wa, eofm_wa, seq='eig') 

    try:
        if ant2_av==False: ant2_av = ant2
        _, wa_l = av_signal(f, mask, step, weight=True, per=per, ant2=ant2_av)
    except:
        print("av_signal failed") 
        wa_l = 10*[np.nan]
  
    add_pc_plot(dat_wa, wa_l, par='tpar', name='p value', sv=nam+'_w_eig', s=10)                             
    add_pc_plot(dat_wa, wa_l, par='var', sv=nam+'_w_eig', s=10) 
    add_pc_plot(dat_wa, wa_l, par='m', sv=nam+'_w_eig', s=10) 
    add_pc_plot(dat_wa, wa_l, par='a', sv=nam+'_w_eig', s=10) 
    add_pc_plot(dat_wa, wa_l, par='d', sv=nam+'_w_eig', s=10) 

    plot_pc_var(var_wa, pc_wa, sv=nam, s=15)
 
    print(seq) 
    df = print_pc_seas(f, nam, mask, step=step, npcs=27, ant2=ant2, ant=ant, seq=seq, ant2_av=ant2_av) 
    df['m2'] = np.where(df['s'] != 'whole', df['m']/4, df['m']) 

    bar_plot(df, sv='step1_'+nam+'_1979_2022', ax1l=[0,0.6], ax2l=0.6) 
    p_bar_plot(df, s=21, sv='step1_'+nam+'_1979_2022') 

    ###

    pcv = [np.var(x) for x in pc_wa] 
    norm = np.sum(var_wa[:10])/np.sum(pcv[:10]) 
    lam_c2 = np.array(pcv[:8])*norm 

    lam = var_wa[0]
    lamc2 = lam_c2[0]
    m_pc1 = np.array(df['m'][df['l']=='1'])[0]
    m_av  = np.array(df['m'][df['l']=='av'])[0]
    p_pc1 = np.array(df['par'][df['l']=='1'])[0]
    p_av  = np.array(df['par'][df['l']=='av'])[0]

    print(nam, " & ", lam, " & ", lamc2, " & ", m_pc1, " & ", m_av, " & ", p_pc1, " & ", p_av) 
